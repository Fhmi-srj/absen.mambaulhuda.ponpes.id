            ...
            <select id="jadwal_id" class="form-select jadwal-select">
                ...
                @foreach($jadwalList ?? [] as $j)
                    <option value="{{ $j->id }}" data-type="{{ $j->type }}">
                        {{ $j->name }} ({{ substr($j->start_time, 0, 5) }} - {{ substr($j->end_time, 0, 5) }})
                    </option>
                @endforeach
            </select>
        </div>
        ...
        function processAttendance(nomorInduk) {
            ...
            navigator.geolocation.getCurrentPosition(
                function (position) {
                    ...
                    fetch('{{ route("api.attendance.store") }}', {
                        method: 'POST',
                        body: data
                    })
                        ...
                        .then(result => {
                            if (result.success) {
                                ...
                                showResult(false, result.siswa_name || 'Gagal', result.message);
                                addRecentScan(result.siswa_name || nomorInduk, false, result.message);
                            }
